...
public class MySqlShoppingCartDao extends MySqlDaoBase implements ShoppingCartDao {
    ...
    public ShoppingCart getByUserId(int userId) {
        //creates an empty ShoppingCart that fill with results
        ShoppingCart cart = new ShoppingCart();

        //opens database connection and prepare a SQL query
        ...
        {
            //binds  userId to the query parameter
            q.setInt(1, userId);

            //execute  query and receive the result set
            ResultSet r = q.executeQuery();

            //loops each row in the result
            while(r.next()){
                //creates  Product model and set fields using the row data
                ...
                //creates ShoppingCartItem containing product + quantity + user
        ...
        //returns populated ShoppingCart (empty if none found)
        return cart;
    }

    //helper method to convert a database ResultSet row into a Product object
